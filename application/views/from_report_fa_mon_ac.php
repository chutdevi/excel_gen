     ...
     if(count($list_act_report[$key]) > 0 )
         { 
                   if ($key == 'fa_summary') 
                   {
                    ...
                    $objPHPExcel->getActiveSheet()->setAutoFilter('A4:'.$col_name[17].'4');
                    $objPHPExcel->getActiveSheet()
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle('A3:'.$col_name[17].'3')->applyFromArray(array('fill'    => Style_Fill($colhead)));
                    $objPHPExcel->getActiveSheet()->getStyle('A3:'.$col_name[17].'3')->applyFromArray(array('font'    => Style_Font(11, '000000', true, 'Calibri')));
                    $objPHPExcel->getActiveSheet()->getStyle('A5:'.$col_name[17].(count( $list_act_report[$key] )+5))->applyFromArray(array('font'    => Style_Font(10, '000000', false, 'Calibri')));
                    $objPHPExcel->getActiveSheet()->getStyle('A3:'.$col_name[17].'4')->applyFromArray(array('borders' => array('allborders' => Style_border(PHPExcel_Style_Border::BORDER_THIN,'FFFFFF'))));
                    // $objPHPExcel->getActiveSheet()->getStyle("A1")->getFont()->setBold(true)
                    //             ->setName('Consolas')
                    //             ->setSize(11)
                    //             ->getColor()->setRGB('FFFFFF');
                    $objPHPExcel->getActiveSheet()->getStyle('A4:'.$col_name[17].'4')->applyFromArray(array('fill'    => Style_Fill('D9D9D9')));
                    $startData = 5;
                    $r = 5;
                            foreach ($value as $nr => $val) 
                            {
                                $indCol = 0;
                                        foreach ($val as $rowData => $data) 
                                        {
                                         ...
                                         if ($rowData == 'LINE_CD' && $data == 'K1M082')
                                          {
                                               $partset = '=(J'.$r.'*'.'K'.$r.')';
                                               $objPHPExcel->getActiveSheet()->setCellValue('L'.$r, $partset); 
                                          }

                                                 elseif ($rowData == 'LINE_CD' && $data == 'K1M083')
                                          {
                                               $partset = '=(J'.$r.'*'.'K'.$r.')';
                                               $objPHPExcel->getActiveSheet()->setCellValue('L'.$r, $partset);
   
                                          }
                                                 elseif ($rowData == 'LINE_CD' && $data !== 'K1M082')
                                          {
                                               $partset = '=(J'.$r.'+'.'K'.$r.')';
                                               $objPHPExcel->getActiveSheet()->setCellValue('L'.$r, $partset);
        ...
        //echo $Montlast.$Daylast;exit;
                               ...
                               $objPHPExcel->getActiveSheet()->setCellValue('A2', "SUMMARY FROM (  01 ".strtoupper($M)." - ".$Daylast." ".strtoupper($M)."  )");
                        ...
                        //    $objPHPExcel->getActiveSheet()->setCellValue('H1', "Accumulate Production \n [pcs.] ");
                            ...
                            $objPHPExcel->getActiveSheet()->setCellValue('F2', '=SUBTOTAL(9,F5:F'. (count( $list_act_report[$key] )+5) . ")" );
                            $objPHPExcel->getActiveSheet()->setCellValue('G2', '=SUBTOTAL(9,G5:G'. (count( $list_act_report[$key] )+5) . ")" );
                       ...
                       //     $objPHPExcel->getActiveSheet()->setCellValue('I2', '=SUBTOTAL(9,M5:M'. (count( $list_act_report[$key] )+5) . ")" );
                            $objPHPExcel->getActiveSheet()->setCellValue('M2', '=SUBTOTAL(9,M5:M'. (count( $list_act_report[$key] )+5) . ")" );
                            $objPHPExcel->getActiveSheet()->setCellValue('N2', '=SUBTOTAL(9,N5:N'. (count( $list_act_report[$key] )+5) . ")" );
                            $objPHPExcel->getActiveSheet()->setCellValue('O2', '=SUBTOTAL(9,O5:O'. (count( $list_act_report[$key] )+5) . ")" );
                            $objPHPExcel->getActiveSheet()->setCellValue('P2', '=SUBTOTAL(9,P5:P'. (count( $list_act_report[$key] )+5) . ")" );
                            $objPHPExcel->getActiveSheet()->setCellValue('Q2', '=SUBTOTAL(9,Q5:Q'. (count( $list_act_report[$key] )+5) . ")" );
                            $objPHPExcel->getActiveSheet()->setCellValue('R2', '=SUBTOTAL(9,R5:R'. (count( $list_act_report[$key] )+5) . ")" );

                     //        $objPHPExcel->getActiveSheet()->setCellValue('H2', '=SUBTOTAL(9,G5:G'. '/'. '=SUBTOTAL(9,F5:F'. (count( $list_act_report[$key] )+5) . ")" );
...
#126180
                            $objPHPExcel->getActiveSheet()->getStyle('F2:R2')->applyFromArray(array('font'    => Style_Font(12, '000000', true, 'Calibri')));
                    ...
                    //                     ->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);                            
                           ...
                           //      ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); 
                            ...
                            $objPHPExcel->getActiveSheet()->getStyle('F2:R2')
                                                          ->getNumberFormat()->setFormatCode('_* #,##0_-;[RED](#,##0)_-;_* [BLACK]"-"??_-;_-@_-');
...
#---------------------------------------------------------------------------------------------------------------------------------------------
       ...
       //   $objPHPExcel->getActiveSheet()->insertNewColumnBefore('L', 1);
          ...
          // {
          //       $averman = '=(F'.$r.'/'.'G'.$r.')';
          //       $objPHPExcel->getActiveSheet()->setCellValue('H2'.$r, $averman); 
          //    }
            // $minusTime = '((K'.$i.'+'.'L'.$i.')*G'.$i.')/I'.$i;
            // $objPHPExcel->getActiveSheet()->setCellValue('M'.$i, '='.$minusTime);S
          
            $summan = '(SUBTOTAL(9,F5:F'. (count( $list_act_report[$sheetIndex] )+5) . '))'; 
          	$sumworkday = '(SUBTOTAL(9,G5:G'. (count( $list_act_report[$sheetIndex] )+5) . '))'; 
          //  	$SumUse = '(SUBTOTAL(9,K5:K'. (count( $list_act_report[$sheetIndex] )+5) . '))';  //SumUse
           ...
           $objPHPExcel->getActiveSheet()->setCellValue('H2', '=('.$summan.'/'.$sumworkday.')');
...
#---------------------------------------------------------------------------------------------------------------------------------------------
          ...
          // foreach (range('K', 'T') as $key) { $objPHPExcel->getActiveSheet()->getColumnDimension($key)->setWidth('12'); }
            # code...
          $objPHPExcel->getActiveSheet()->getStyle('K2:K'.(count( $list_act_report[$sheetIndex] )+5))->getNumberFormat()->setFormatCode('_(* #,##0_);_(* (#,##0);_(* "-"??_);_(@_)'); 
          $objPHPExcel->getActiveSheet()->getStyle('O2:O'.(count( $list_act_report[$sheetIndex] )+5))->getNumberFormat()->setFormatCode('_(* #,##0_);_(* (#,##0);_(* "-"??_);_(@_)'); 
          $objPHPExcel->getActiveSheet()->getStyle('P2:P'.(count( $list_act_report[$sheetIndex] )+5))->getNumberFormat()->setFormatCode('_(* #,##0_);_(* (#,##0);_(* "-"??_);_(@_)'); 
          $objPHPExcel->getActiveSheet()->getStyle('Q2:Q'.(count( $list_act_report[$sheetIndex] )+5))->getNumberFormat()->setFormatCode('_(* #,##0_);_(* (#,##0);_(* "-"??_);_(@_)'); 
          $objPHPExcel->getActiveSheet()->getStyle('R2:R'.(count( $list_act_report[$sheetIndex] )+5))->getNumberFormat()->setFormatCode('_(* #,##0_);_(* (#,##0);_(* "-"??_);_(@_)'); 


          $objPHPExcel->getActiveSheet()->getStyle('I2:I'.(count( $list_act_report[$sheetIndex] )+5))->getNumberFormat()->setFormatCode('_* #,##0_-;[RED](#,##0)_-;_* [BLACK]"-"??_-;_-@_-'); 
          $objPHPExcel->getActiveSheet()->getStyle('J2:J'.(count( $list_act_report[$sheetIndex] )+5))->getNumberFormat()->setFormatCode('_* #,##0_-;[RED](#,##0)_-;_* [BLACK]"-"??_-;_-@_-');  
          $objPHPExcel->getActiveSheet()->getStyle('L2:L'.(count( $list_act_report[$sheetIndex] )+5))->getNumberFormat()->setFormatCode('_* #,##0_-;[RED](#,##0)_-;_* [BLACK]"-"??_-;_-@_-');  
          $objPHPExcel->getActiveSheet()->getStyle('O2:O'.(count( $list_act_report[$sheetIndex] )+5))->getNumberFormat()->setFormatCode('_* #,##0_-;[RED](#,##0)_-;_* [BLACK]"-"??_-;_-@_-');  

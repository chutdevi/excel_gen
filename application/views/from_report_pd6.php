...
#========================================================================================================================  Put field ====================================================================================        
                ...
                $st_dat = 14;
                $sub = 6;
                $count_index =  ( count($list_act_report[$sheetIndex][0])-1 )  - ( 31 - $lastmount ) ;
                $row = $st_dat;
...
#========================================================================================================================  Put data ====================================================================================                
                   ...
                   $col = 2;
                    foreach ($value as $body => $val) 
                    {
                        ...
                        if ($val == "1" && $body == "H" )
                        {
                            ...
                            $objPHPExcel->getActiveSheet()->setCellValue('F'.$row,  '  Plan');


                            $objPHPExcel->getActiveSheet()->setCellValue($col_name[($count_index+3)].$row,  '  Plan');
                            $objPHPExcel->getActiveSheet()->setCellValue( $col_name[($count_index+4)].$row, '=SUM(' .$col_name[6].$row.":".$col_name[(6+$lastmount)].$row.")");
                
             
                
                            $objPHPExcel->getActiveSheet()->getStyle( $col_name[($count_index+3)].$row .':'. $col_name[($count_index+4)].($row+4) )
                                                          ->applyFromArray(array(
                                                            'borders' => array('inside'   => Style_border(PHPExcel_Style_Border::BORDER_THIN,'a6a6a6'))));                
                           // $objPHPExcel->getActiveSheet()->mergeCells($col_name[40].($row-1).':'.$col_name[40].$row);

                        }
                        elseif($val == "2" && $body == "H")
                        {

                            $objPHPExcel->getActiveSheet()->setCellValue('F'.$row,  '  Actual');

                            $objPHPExcel->getActiveSheet()->getStyle('F'. $row .':'. $col_name[$count_index] . $row)->applyFromArray(array('fill' => Style_Fill('d9ffb3')));
                            $objPHPExcel->getActiveSheet()->getStyle('F'. $row .':'. $col_name[$count_index] . $row)->applyFromArray(array('font' => Style_Font(10,"0000ff",true,true)));


                            $objPHPExcel->getActiveSheet()->setCellValue($col_name[($count_index+3)].$row,  '  Actual');
                            $objPHPExcel->getActiveSheet()->setCellValue( $col_name[($count_index+4)].$row, '=SUM(' .$col_name[6].$row.":".$col_name[(6+$lastmount)].$row.")");

                            $objPHPExcel->getActiveSheet()->getStyle($col_name[($count_index+3)].$row .':'. $col_name[($count_index+4)].$row)->applyFromArray(array('fill' => Style_Fill('d9ffb3')));
                            $objPHPExcel->getActiveSheet()->getStyle($col_name[($count_index+3)].$row .':'. $col_name[($count_index+4)].$row)->applyFromArray(array('font' => Style_Font(10,"0000ff",true,true)));  
                        ...
                        elseif($val == "3" && $body == "H")
                        {
                            ...
                            $objPHPExcel->getActiveSheet()->setCellValue($col_name[($count_index+3)].$row,  '  Diff');
                            $objPHPExcel->getActiveSheet()->setCellValue( $col_name[($count_index+4)].$row, '=SUM(' .$col_name[6].$row.":".$col_name[(6+$lastmount)].$row.")");
                        ...
                        elseif($val == "4" && $body == "H")
                        {
                            ...
                            $objPHPExcel->getActiveSheet()->setCellValue('F'.$row,  '  Acc. Diff');


                            $objPHPExcel->getActiveSheet()->setCellValue($col_name[($count_index+3)].$row, '  Acc. Diff');
                            //$objPHPExcel->getActiveSheet()->setCellValue( $col_name[($count_index+4)].$row, '=' . $col_name[(6+$lastmount)].($row-1) );

                            $objPHPExcel->getActiveSheet()->mergeCells( $col_name[($count_index+4)].($row-1).':'.$col_name[($count_index+4)].$row );
                        ...
                        elseif($val == "5" && $body == "H")
                        {
                            ...
                            $objPHPExcel->getActiveSheet()->setCellValue('F'.$row,  '  Ng');
                            ...
                            $objPHPExcel->getActiveSheet()->getStyle('F'. $row .':'. $col_name[$count_index] . $row)->applyFromArray(array('fill' => Style_Fill('eaeae1')));
                            $objPHPExcel->getActiveSheet()->getStyle('F'. $row .':'. $col_name[$count_index] . $row)->applyFromArray(array('font' => Style_Font(10,"ff0000",true,true)));
                            //$objPHPExcel->setActiveSheetIndex($ind)->setCellValue($col_name[$col++].($row), $val);


                            $objPHPExcel->getActiveSheet()->setCellValue($col_name[($count_index+3)].$row, '  Ng');      
                            $objPHPExcel->getActiveSheet()->setCellValue( $col_name[($count_index+4)].$row, '=SUM(' .$col_name[6].$row.":".$col_name[(6+$lastmount)].$row.")");

                            $objPHPExcel->getActiveSheet()->getStyle($col_name[($count_index+3)].$row .':'. $col_name[($count_index+4)].$row)->applyFromArray(array('fill' => Style_Fill('eaeae1')));
                            $objPHPExcel->getActiveSheet()->getStyle($col_name[($count_index+3)].$row .':'. $col_name[($count_index+4)].$row)->applyFromArray(array('font' => Style_Font(10,"ff0000",true,true)));  

                          //  $objPHPExcel->getActiveSheet()->mergeCells($col_name[40].($row-1).':'.$col_name[40].$row);

                            $objPHPExcel->getActiveSheet()->getStyle($col_name[($count_index+3)].$row .':'. $col_name[($count_index+4)].$row)
                                                          ->applyFromArray(array(
                                                            'borders' => array('bottom' => Style_border(PHPExcel_Style_Border::BORDER_THICK,'000000'))));
                        }  

                        //echo 
                        if( $col > 4 && strlen($body) == 4 )
                        {
                            $in = intval(substr($body, 0,2)) - 1;

                            if ($value['H'] == '1')
                            {
                                $subplan[$in] .= "," . $col_name[($in+6)].($row);
                                $objPHPExcel->getActiveSheet()->setCellValue( $col_name[($in+6)] . '7', $subplan[$in] . ")");
                            }

                            elseif ($value['H'] == '2')
                            {
                                $subactual[$in] .= "," . $col_name[($in+6)].($row);
                                $objPHPExcel->getActiveSheet()->setCellValue( $col_name[($in+6)] . '8', $subactual[$in] . ")");
                            }

                            elseif ($value['H'] == '3')
                            {
                                $subdiff[$in] .= "," . $col_name[($in+6)].($row);
                                $objPHPExcel->getActiveSheet()->setCellValue( $col_name[($in+6)] . '9', $subdiff[$in] . ")");
                            }


                            elseif ($value['H'] == '4')
                            {
                                $subacc_diff[$in] .= "," . $col_name[($in+6)].($row);
                                $objPHPExcel->getActiveSheet()->setCellValue( $col_name[($in+6)] . '10', $subacc_diff[$in] . ")");
                            }


                            elseif ($value['H'] == '5')
                            {
                                $subng[$in] .= "," . $col_name[($in+6)].($row);
                                $objPHPExcel->getActiveSheet()->setCellValue( $col_name[($in+6)] . '11', $subng[$in] . ")");
                            }                                                                                    
 ...
 //echo $til . " = " . $subplan[0] . "<hr>" ;
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('C2', 'Production of '.date('F Y'));
...
//echo $col_name[42]; exit;
                ...
                $objPHPExcel->getActiveSheet()->getStyle('G'.'7'.':'.$col_name[$count_index+6].'11')
                                              ->getNumberFormat()->setFormatCode('_-* #,##0_-;[Red](#,##0)_-;_-* "-"??_-;_-@_-');
                ...
                $objPHPExcel->getActiveSheet()->getColumnDimension($col_name[2])->setWidth('16.71');    #I model
                $objPHPExcel->getActiveSheet()->getColumnDimension($col_name[3])->setWidth('26.71');    #J
                $objPHPExcel->getActiveSheet()->getColumnDimension($col_name[4])->setWidth('20.71');    #K
                ...
                foreach(range(6, $count_index) as $r)
               ...
               // Style_Alignment('J3',3, false, $objPHPExcel);
                ...
                $objPHPExcel->getActiveSheet()->mergeCells('C7:'.'E11');
                ...
                elseif ( date('d') >= 3 && date('d') < $lastmount ) 
                    {
                        foreach (range( 6 , ( (date('d')-3)+6 ) ) as $index) Style_group_lv1_Col($col_name, $index, $objPHPExcel );

                        foreach (range( ((date('d')+3)+6) , $count_index ) as $index) Style_group_lv1_Col($col_name, $index, $objPHPExcel );

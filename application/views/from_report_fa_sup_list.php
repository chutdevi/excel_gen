     ...
     if(count($list_act_report[$sheetIndex]) > 0 )
         { 
                   ...
                   $objPHPExcel->setActiveSheetIndex($indSheet);
                    ...
                    $objPHPExcel->getActiveSheet()->getRowDimension( 5 )->setRowHeight( 5 );
                    ...
                    $objPHPExcel->getActiveSheet()
                                ->getStyle(('D'.$st_cal.':'.$col_name[21].$st_cal))
                                ->getAlignment()
                                ->setWrapText(true)
                                ->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER)
                                ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); 
                    ...
                    $objPHPExcel->getActiveSheet()->setAutoFilter('D8:'.$col_name[7].'8');
                    $objPHPExcel->getActiveSheet()
                                ->getStyle('D9:'.$col_name[21].(count( $list_act_report[$sheetIndex] )+8))
                                ->getAlignment()
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle('D6:'.$col_name[21].'6')->applyFromArray(array('fill'    => Style_Fill($colhead)));
                    $objPHPExcel->getActiveSheet()->getStyle('D6:'.$col_name[21].'6')->applyFromArray(array('font'    => Style_Font(10, 'FFFFFF', true, 'Arial Narrow')));
                    $objPHPExcel->getActiveSheet()->getStyle('D9:'.$col_name[21].(count( $list_act_report[$sheetIndex] )+8))->applyFromArray(array('font'    => Style_Font(11, '000000', false, 'Calibri')));
                    $objPHPExcel->getActiveSheet()->setCellValue('K6', 'STOCK TODAY');
                    //$objPHPExcel->getActiveSheet()->getStyle('D6:'.$col_name[12].'7')->applyFromArray(array('borders' => array('allborders' => Style_border(PHPExcel_Style_Border::BORDER_THIN,'FFFFFF'))));
                    // $objPHPExcel->getActiveSheet()->getStyle("A1")->getFont()->setBold(true)
                    //             ->setName('Consolas')
                    //             ->setSize(11)
                    //             ->getColor()->setRGB('FFFFFF');
                   
                    $objPHPExcel->getActiveSheet()->getStyle('D8:'.$col_name[21].'8')->applyFromArray(array('fill'    => Style_Fill('b3cccc')));
                                ...
                                $indCol = 3;
                                        foreach ($val as $rowData => $data) 
                                        {
                                           if($rowData != 'NO')
                                           {
                                            ...
                                            $objPHPExcel->getActiveSheet()->getStyle('D'.($r).':'.$col_name[21].($r))->applyFromArray(array('fill'    => Style_Fill('d9d9d9'))); //color sheet
                                            $objPHPExcel->getActiveSheet()->getRowDimension( $r )->setRowHeight( 20 );
                                            $objPHPExcel->getActiveSheet()->getStyle('D'.($r).':'.$col_name[21].($r))->applyFromArray(array('font'    => Style_Font(12, '000000', true, 'Calibri')));
                                            }

                                            if ($val['STOCK_TODAY'] < $val['PLAN_QTY'] )
                                            {

                                            	$objPHPExcel->getActiveSheet()->getStyle('D'.($r).':'.$col_name[21].($r))->applyFromArray(array('fill'    => Style_Fill('ffffcc')));
                                            	$objPHPExcel->getActiveSheet()->getStyle('D'.($r).':'.$col_name[21].($r))->applyFromArray(array('font'    => Style_Font(11, 'ff0000', false, 'Calibri')));
                                            }
                                            if ($val['SUP_FROM'] == '')
                                            {
                                            	$objPHPExcel->getActiveSheet()->getStyle('D'.($r).':'.$col_name[21].($r))->applyFromArray(array('fill'    => Style_Fill('d9d9d9')));
                                            	$objPHPExcel->getActiveSheet()->getStyle('D'.($r).':'.$col_name[21].($r))->applyFromArray(array('font'    => Style_Font(11, '000000', true, 'Calibri')));
        ...
        //echo $Montlast.$Daylast;exit;
                            $objPHPExcel->getActiveSheet()->setCellValue('D3', "ใบเตรียมชิ้นงานล่วงหน้าสำหรับการผลิต"); //.strtoupper(date('d-M-Y',  strtotime((date('d')+1) . "-" . date('M') . "-" . date('Y')) ))); //('D2', "FA Supply list of ".$Montlast);
                            $objPHPExcel->getActiveSheet()
                                ->getStyle(('D3'.':'.$col_name[11].'3'))
                                ->getAlignment()
                                ->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER)
                                ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); 
                            $objPHPExcel->getActiveSheet()->setCellValue('D4', "DAILY PART SUPPLY PREPARATION CHECK LIST");
                            $objPHPExcel->getActiveSheet()
                                ->getStyle(('D4'.':'.$col_name[11].'4'))
                                ->getAlignment()
                                ->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER)
                                ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); // example title 
                            $objPHPExcel->getActiveSheet()->setCellValue('S3', "PRODUCTION DATE \n".strtoupper(date('d-M-Y',  strtotime((date('d')+1) . "-" . date('M') . "-" . date('Y')) )));
...
#126180
                      //      $objPHPExcel->getActiveSheet()->getStyle('G2:L2')->applyFromArray(array('font'    => Style_Font(14, '000000', true, 'Calibri')));
        					$objPHPExcel->getActiveSheet()->setCellValue('U6', "เศษ");
                            $objPHPExcel->getActiveSheet()->getStyle('D3')->applyFromArray(array('font'    => Style_Font(26, '002d4d', true, 'AngsanaUPC')));
                            $objPHPExcel->getActiveSheet()->getStyle('D4')->applyFromArray(array('font'    => Style_Font(18, '002d4d', true, 'Arial Narrow')));
                            $objPHPExcel->getActiveSheet()->getStyle('M3')->applyFromArray(array('font'    => Style_Font(16, '002d4d', true, 'Arial Narrow')));
                            $objPHPExcel->getActiveSheet()->getStyle('N3')->applyFromArray(array('font'    => Style_Font(11, '002d4d', true, 'Arial Narrow')));
                            $objPHPExcel->getActiveSheet()->getStyle('S3')->applyFromArray(array('font'    => Style_Font(11, '002d4d', true, 'Arial Narrow')));
                           
                            $objPHPExcel->getActiveSheet()->getStyle('B2:'.'W'.($cu_dat+11))->applyFromArray(array(
                                                                     'borders' => array('outline' => Style_border(PHPExcel_Style_Border::BORDER_THICK,'000000'))));
                            $objPHPExcel->getActiveSheet()->getStyle('D9:'.'V'.($cu_dat+9))->applyFromArray(array(
                                                                     'borders' => array('allborders' => Style_border(PHPExcel_Style_Border::BORDER_THIN,'c2c2d6'))));
                          

                 

                        //  $objPHPExcel->getActiveSheet()->getColumnDimension('Q')->setVisible(false); //hide
                          $objPHPExcel->getActiveSheet()->getColumnDimension('O')->setVisible(false); //hide
                      ...
                      //    $objPHPExcel->getActiveSheet()->getColumnDimension('T')->setVisible(false); //hide
                          ...
                          $objPHPExcel->getActiveSheet()->mergeCells('S3:V3');

                       						  $objPHPExcel->getActiveSheet()->getStyle('J9'.':K'.( count( $list_act_report[$sheetIndex] )+8) )

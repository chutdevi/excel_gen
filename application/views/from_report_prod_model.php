...
#========================================================================================================================  Put data ====================================================================================                
 ...
 //echo $til . " = " . $subplan[0] . "<hr>" ;
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('D2', 'Production of '.date('F Y'));
...
//echo $col_name[42]; exit;
               ...
               // Style_Alignment('J3',3, false, $objPHPExcel);
                ...
                elseif ( (date('d')+0) >= 3 && (date('d')+0) < ($lastmount+0) ) 
                    {

                    	//echo date('d'); exit;
                        	foreach (range( 9 , ( (date('d')-3)+9 ) ) as $index) Style_group_lv1_Col($col_name, $index, $objPHPExcel );



                        foreach (range( ((date('d')+3)+9) , $count_index ) as $index) Style_group_lv1_Col($col_name, $index, $objPHPExcel );
                                                                         
                    }
                else
                    {
                    	//echo ((date('d')+0) > 3) && ((date('d')+0) < ($lastmount+0)); exit;
...
$objPHPExcel->removeSheetByIndex(count($title));                             
    ...
    $myfile_Log = fopen("G:/mha_log/"."com_".date("Ymd").".mha", 'w') or die("Unable to open file!");
    

    ///file_get_contents('http://192.168.161.102/dep_trainer/Api_tool/api_test');
    //echo $str; exit;
    fwrite($myfile_Log,"MhA");
    fclose($myfile_Log);

          ...
          foreach ($list_act_report[$sheetIndex] as $key => $value) {
              ...
              foreach ($value as $body => $val) 
              {
                      ...
                      if ($body == 'LOSS') {
                        ...
                        $EFF = ($ckminustime==0) ? 1:$ckminustime/intval ($value['USE_TIME']);
                        //echo $EFF*100;exit;
            ...
            //  exit;
              ...
              $objPHPExcel->getActiveSheet()->freezePane('K2');
                    ...
                    $objPHPExcel->getActiveSheet()->insertNewRowBefore(1,2);
                            ...
                            $objPHPExcel->getActiveSheet()->getStyle('T1:Y1')->applyFromArray(array('fill'    => Style_Fill('92DCC9')));
                            $objPHPExcel->getActiveSheet()->getStyle('K2:S2')->applyFromArray(array('fill'    => Style_Fill('FFCCCC')));
                            $objPHPExcel->getActiveSheet()->getStyle('T2:Y2')->applyFromArray(array('fill'    => Style_Fill('CCFFFF')));
...
#---------------------------------------------------------------------------------------------------------------------------------------------
          ...
          $objPHPExcel->getActiveSheet()->setCellValue('U3', '[G] Model Change');
          $objPHPExcel->getActiveSheet()->setCellValue('V3', '[H] Change the Tooling.');
          $objPHPExcel->getActiveSheet()->setCellValue('W3', '[K] The machine is broken.');
          $objPHPExcel->getActiveSheet()->setCellValue('X3', '[L] Customize the size.');
          $objPHPExcel->getActiveSheet()->setCellValue('Y3', '[N] The store does not have a watson.');
          $objPHPExcel->getActiveSheet()->setCellValue('Z3', '[R] But the cruise the element.');
...
#-------------------------------------------------------------------------------------------------------------------------------------------
          ...
          $objPHPExcel->getActiveSheet()->getStyle('T2:T'.(count( $list_act_report[$sheetIndex] )+5))->getNumberFormat()->setFormatCode('_-* #,##0.00%_-;[Red](#,##0.00%)_-;_-* "-"??_-;_-@_-'); 

...
foreach ($title as $inTil => $til) 
{
     ...
     if(count($list_act_report[$sheetIndex]) > 0 )
     {
        foreach ( $list_act_report[$sheetIndex][0] as $key => $val ) 
        {
            ...
            {
                $objPHPExcel->setActiveSheetIndex($inTil)->setCellValue($col_name[$i-1]."1", str_replace("_", " ", date('d M', strtotime(($day++)."-".date('M')."-".date('Y')) )));
     ...
     if(count($list_act_report[$key]) > 0 )
         { 
                   if ($key == 'defect_daily') 
                   {
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle('A3:'.$col_name[date('d')+10].'3')->applyFromArray(array('font'    => Style_Font(10,'FFFFFF',true)));
                    $objPHPExcel->getActiveSheet()->getStyle('A3:'.$col_name[date('d')+10].'3')->applyFromArray(array('fill'    => Style_Fill($colhead)));
                    $objPHPExcel->getActiveSheet()->getStyle('A3:'.$col_name[date('d')+10].'3')->applyFromArray(array('borders' => array('allborders' => Style_border(PHPExcel_Style_Border::BORDER_THIN,'FFFFFF')))); 
                    ...
                    $r = 4;
                                           ...
                                           if($indCol > 11 && $r == 4)
                                           {
                                            ...
                                            else
                                            {
                                               $objPHPExcel->setActiveSheetIndex($indSheet)->setCellValue($col_name[$indCol++].($r), $data);
                                                    ...
                                                    {
                                                        if (substr($rowData,4,2) == (date('d')-1)) 
                                                        {
                                                            break;
                                                        }
                                                        else
                            ...
                            $objPHPExcel->getActiveSheet()->getStyle('A1:'.$col_name[date('d')+10].'2')->applyFromArray(array('borders' => array('allborders' => Style_border(PHPExcel_Style_Border::BORDER_THIN,'FFFFFF'))));
                            ...
                            $objPHPExcel->getActiveSheet()->getStyle('L1:'.$col_name[date('d')+10].'1' )
                                                          ->getNumberFormat()->setFormatCode('_* #,##0_-;[RED](#,##0)_-');

                            $objPHPExcel->getActiveSheet()->getStyle('L2:'.$col_name[date('d')+10].(count( $list_act_report[$key] )+5) )
                                                          ->getNumberFormat()->setFormatCode('_-* #,##0_-;[RED](#,##0)_-;_-* [BLACK]"-"??_-;_-@_-');

                            $objPHPExcel->getActiveSheet()->getStyle('M4:'.$col_name[date('d')+10].(count( $list_act_report[$key] )+5))->applyFromArray(array('font' => Style_Font(10,'FF0000',true)));

                            $objPHPExcel->getActiveSheet()->getStyle('L1:'.$col_name[date('d')+10].'1')->applyFromArray(array('font' => Style_Font(16,'FFFFFF',true)));
                            $objPHPExcel->getActiveSheet()->getStyle('L1:'.$col_name[date('d')+10].'1')->getFont()->setUnderline(true);

                            $objPHPExcel->getActiveSheet()->getStyle('L2:'.'L'.$col_name[date('d')+10].'2')->applyFromArray(array('font' => Style_Font(11,'FF0000',true)));
                            ...
                            foreach (range('A', 'Q') as $key) $objPHPExcel->getActiveSheet()->getColumnDimension('A'.$key)->setWidth('11');


$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setVisible(false);
$objPHPExcel->getActiveSheet()->getColumnDimension('F')->setVisible(false);
$objPHPExcel->getActiveSheet()->getColumnDimension('H')->setVisible(false);
$objPHPExcel->getActiveSheet()->getStyle('A1')->applyFromArray(array('font' => Style_Font(36,'000000',true)));
...
function sunday($dat = '01-01-2018',  $focus = 1, $start_row = 4, $end_row = 5, $col = null, $ind = 0,  $objPHPExcel = nul)
{
    ...
    {        
        ...
        $Tday = $valDay."-".date('M')."-".date('Y');
        $Nday = date('l', strtotime($Tday));
//echo date('d', strtotime($Tday)) . "-" . $Nday . $col[($d+11)] . ":" . $col[$indexSun] . "<hr>";  exit;
        if($Nday == 'Sunday')
        {            

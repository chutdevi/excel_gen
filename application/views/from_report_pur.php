...
$data_col = array();
//svar_dump($list_act_report); exit;
...
foreach ($title as $inTil => $til) 
{
    ...
    {      
            ...
            //$objPHPExcel->setActiveSheetIndex($inTil)->setCellValue("A1", "INVOICE DATE");
            //exit;
            $row = 3;
            foreach ($list_act_report[$sheetIndex] as $key => $value) 
            {
                ...
                foreach ($value as $body => $val)
                {
                        ...
                        if( $body == 'PUCH_ODR_CD' &&  $val == '') 
                        {
                            $objPHPExcel->setActiveSheetIndex($ind)->setCellValue("A".($row), "INVOICE TOTAL");
                            $objPHPExcel->getActiveSheet()->getRowDimension( $row )->setRowHeight( 18 );
                            $objPHPExcel->getActiveSheet()->getStyle('A'.$row.':'.$col_name[$count_index].$row)->applyFromArray(array('fill'    => Style_Fill('FFFFCC')));
                            $objPHPExcel->getActiveSheet()->getStyle('A'.$row.':'.$col_name[$count_index].$row)->applyFromArray(array('font' => Style_Font(10,'000000',true,'Ebrima')));

                        }

                        if( $body == 'VEND_CD' &&  $val == '') 
                        {
                            $objPHPExcel->setActiveSheetIndex($ind)->setCellValue("A".($row), "VENDOR TOTAL");
                            $objPHPExcel->getActiveSheet()->getRowDimension( $row )->setRowHeight( 25 );
                            $objPHPExcel->getActiveSheet()->getStyle('A'.$row.':'.$col_name[$count_index].$row)->applyFromArray(array('fill'    => Style_Fill('dce6f0')));
                            $objPHPExcel->getActiveSheet()->getStyle('A'.$row.':'.$col_name[$count_index].$row)->applyFromArray(array('font' => Style_Font(11,'000000',true,'Ebrima')));
            ...
            //$objPHPExcel->getActiveSheet()->getStyle('A3:A'.$count_data)->getNumberFormat()->setFormatCode('_-* #,##0_-;[RED](#,##0)_-;_-* "-"??_-;_-@_-');
            $objPHPExcel->getActiveSheet()->getStyle('H3:H'.$count_data)->getNumberFormat()->setFormatCode('_-* #,##0_-;[RED](#,##0)_-;_-* "-"??_-;_-@_-');
            //$objPHPExcel->getActiveSheet()->getStyle('H3:H'.$count_data)->getNumberFormat()->setFormatCode('_-* #,##0_-;[RED](#,##0)_-;_-* "-"??_-;_-@_-');
            $objPHPExcel->getActiveSheet()->getStyle('J3:M'.$count_data)->getNumberFormat()->setFormatCode('_-* #,##0.00_-;[RED](#,##0.00)_-;_-* "-"??_-;_-@_-');
...
// //$objWriter->setUseDiskCaching(true); //create tmp cache
// $objWriter->save('php://output');
// // $con = 'ship_remain'.date('d').'.xlsx';
// // $objWriter->save('D:/'.$con);

$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
$objWriter->save('G:/file_export/purchase_report/'.$filename.'-'.date('dmy').'.xlsx');


output_file("G:/file_export/purchase_report/".$filename.'-'.date('dmy').'.xlsx');
//exit;

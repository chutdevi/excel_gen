       ...
       foreach (range('I', 'J') as $key) $objPHPExcel->getActiveSheet()->getColumnDimension($key)->setWidth('12'); #ขนาดคอลัม    
          ...
          foreach ($list_act_report[$sheetIndex] as $key => $value) {
              ...
              foreach ($value as $body => $val) 
              {
                     ...
                     if ($body == 'PRODUCT_TYPE' && $val == '10')
                      {
                           $fgmanhr = '(AD'.$row.')';
                           $objPHPExcel->getActiveSheet()->setCellValue('AS'.$row, '='.$fgmanhr);
                          //echo $minusTime. "<hr>" ; 
                          //echo $minusTime ; exit;                          
                    ...
                    //  var_dump($value); exit;
                      ...
                      if ($value['ACTUAL_CT'] > $value['STD_CT'] && $body == 'STD_CT')
                      {
                        $objPHPExcel->getActiveSheet()->getStyle('M'.($row))->applyFromArray(array('fill'    => Style_Fill('ffff00')));
                        $objPHPExcel->getActiveSheet()->getStyle('M'.($row))->applyFromArray(array('font'    => Style_Font(11, '000000', false, 'Calibri')));                          
               ...
               $objPHPExcel->getActiveSheet()->setCellValue('A'.$row, sprintf('%s',$row-1));

               $objPHPExcel->getActiveSheet()->setCellValue('AT'.$row, '=IF(C'.($row-1).'<>'.'C'.$row.',1,0)');

               $objPHPExcel->getActiveSheet()->setCellValue('AU'.$row, sprintf('=IF((C%s&F%s)<>(C%s&F%s),E%s,0 )',($row),($row),($row-1),($row-1),$row));

               $objPHPExcel->getActiveSheet()->setCellValue('AV'.$row, sprintf('=IF((C%s&F%s)<>(C%s&F%s),1,0 )',($row),($row),($row-1),($row-1)));



            //   $objPHPExcel->getActiveSheet()->setCellValue('AU'.$row, '=IF(C'.($row-1).'<>'.'C'.$row.',1,0)');
             // exit;
             // var_dump($value); 
            //  exit;
              $row++;
              $objPHPExcel->getActiveSheet()->setAutoFilter($col_name[0]."1:AR1");
...
//exit;
                    $objPHPExcel->getActiveSheet()->insertNewRowBefore(1,2);
                            ...
                            $objPHPExcel->getActiveSheet()->setCellValue('AT3', "Line");
                            $objPHPExcel->getActiveSheet()->setCellValue('AU3', "Staff");
                            $objPHPExcel->getActiveSheet()->setCellValue('AV3', "Shift");
                      

                            $objPHPExcel->getActiveSheet()->getStyle('A1')->applyFromArray(array('font' => Style_Font(42,'000000',true)));
                            $objPHPExcel->getActiveSheet()->getStyle('L1')->applyFromArray(array('font' => Style_Font(18,'000000',true)));
                            $objPHPExcel->getActiveSheet()->getStyle('AA1:AC1')->applyFromArray(array('font' => Style_Font(11,'000000',false)));
                            $objPHPExcel->getActiveSheet()->getStyle('AE1:AQ1')->applyFromArray(array('font' => Style_Font(18,'000000',true)));
                            $objPHPExcel->getActiveSheet()->getStyle('M1:U1')->applyFromArray(array('font' => Style_Font(18,'000000',true)));
                    ...
                    $objPHPExcel->getActiveSheet()->setCellValue('C2', '=SUBTOTAL(9,AT4:AT'. (count( $list_act_report[$sheetIndex] )+5) . ")");
                    $objPHPExcel->getActiveSheet()->setCellValue('E2', '=SUBTOTAL(9,AU4:AU'. (count( $list_act_report[$sheetIndex] )+5) . ")");
                    $objPHPExcel->getActiveSheet()->setCellValue('F2', '=SUBTOTAL(9,AV4:AV'. (count( $list_act_report[$sheetIndex] )+5) . ")");


                    // $objPHPExcel->getActiveSheet()->setCellValue('E2', '=SUBTOTAL(9,E4:E'. (count( $list_act_report[$sheetIndex] )+5) . ")");
                ...
                //   $objPHPExcel->getActiveSheet()->setCellValue('S2', '=SUBTOTAL(9,S4:S'. (count( $list_act_report[$sheetIndex] )+5) . ")");
                 ...
                 //   $objPHPExcel->getActiveSheet()->setCellValue('AS1', '=SUBTOTAL(9,AS4:AS'. (count( $list_act_report[$sheetIndex] )+5) . ")");
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle('AD1')->getNumberFormat()->setFormatCode('_-* #,##0.00_-;[RED](#,##0.00)_-;_-* "-"??_-;_-@_-'); //IMPORT
                 //   $objPHPExcel->getActiveSheet()->getStyle('AS')->getNumberFormat()->setFormatCode('_-* #,##0.00_-;[RED](#,##0.00)_-;_-* "-"??_-;_-@_-'); //IMPORT
...
// #----------------------------------------------------------------------------------Man Hr Pcs (FG)---------------------------------------------------------
          ...
          $Sumtime_allman = '(SUBTOTAL(9,AD4:AD'. (count( $list_act_report[$sheetIndex] )+5) . '))';

          $Summan = '(SUBTOTAL(9,E4:E307'. (count( $list_act_report[$sheetIndex] )+5) . '))';
          $subfg = '(SUBTOTAL(9,AS4:AS'. (count( $list_act_report[$sheetIndex] )+5) . '))';
     ...
     //     $Sumactual = '(SUBTOTAL(9,N4:N272'. (count( $list_act_report[$sheetIndex] )+5) . '))';
          ...
          $objPHPExcel->getActiveSheet()->setCellValue('Z1', '='.$SumACTUAL);
          $objPHPExcel->getActiveSheet()->setCellValue('AB1', '='.$Sumtime_allman);

           $objPHPExcel->getActiveSheet()->setCellValue('AD1', '=('.'AB1'.'/'.'Z1'.')');

	      $objPHPExcel->getActiveSheet()->setCellValue('AB3', 'EFF.(%)');
          $objPHPExcel->getActiveSheet()->setCellValue('AD3', 'TOTALTIME ALL MAN');
          $objPHPExcel->getActiveSheet()->setCellValue('X1', ' ACTUAL (FG) >>');
          $objPHPExcel->getActiveSheet()->setCellValue('AA1', 'TIME ALL MAN (FG) >>');
          $objPHPExcel->getActiveSheet()->setCellValue('AC1', 'Man Hr Pcs ( FG )>>');
...
// #---------------------------------------------------------------------------EFF % WORKING TIME----------------------------------------------------------------
    ...
    ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);      
	   ...
	   $objPHPExcel->getActiveSheet()->getStyle('Z1:AB1'.(count( $list_act_report[$sheetIndex] )+5))->getNumberFormat()->setFormatCode('_- #,##0_-;- #,##0_-;_-* "-"??_-;_-@_-');
     ...
     //   $objPHPExcel->getActiveSheet()->getStyle('AC2'.(count( $list_act_report[$sheetIndex] )+5))->getNumberFormat()->setFormatCode('_-* #,##0%_-;[Red](#,##0%)_-;_-* "-"??_-;_-@_-');
        ...
        //   $objPHPExcel->getActiveSheet()->getColumnDimension('P')->setVisible(false); //hide
          ...
          $objPHPExcel->getActiveSheet()->mergeCells('L1:U1');

...
#======================================================================== Fill set ====================================================================================                    
                foreach ( $holiday as $ind_cal => $val)
                {
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle( $col_name[ (int)substr( $val['d_t'],-2 ) + 9 ] . $st_dat  . ':' .  $col_name[ (int)substr( $val['d_t'],-2 ) + 8 ] . $count_data )->applyFromArray(array( 'fill' => Style_Fill('BFC9CA') ));


                    $objPHPExcel->getActiveSheet()->getStyle( $col_name[ (int)substr( $val['d_t'],-2 ) + 9 ] . '4'  . ':' .  $col_name[ (int)substr( $val['d_t'],-2 ) + 8 ] . '13' )->applyFromArray(array( 'fill' => Style_Fill('BFC9CA') ));


                    $objPHPExcel->getActiveSheet()->getStyle( $col_name[ (int)substr( $val['d_t'],-2 ) + 9 ] . $st_dat  . ':' .  $col_name[ (int)substr( $val['d_t'],-2 ) + 8 ] . $count_data )->applyFromArray(array('font' => Style_Font(11,"000FFF",False,False,'Calibri')));

                    $objPHPExcel->getActiveSheet()->getStyle( $col_name[ (int)substr( $val['d_t'],-2 ) + 9 ] . '4'  . ':' .  $col_name[ (int)substr( $val['d_t'],-2 ) + 8 ] . '13' )->applyFromArray(array('font' => Style_Font(11,"000FFF",False,False,'Calibri')));
                }
              if( $del == 0 )
              {
                if(date('d') != '01')
